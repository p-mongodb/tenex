...
Routes.included do
  ...
  get '/repos/:org/:repo/pulls/:id' do |org_name, repo_name, id|
    ...
    @configs = {
      'mongodb-version' => %w(6.0 5.3 5.2 5.0 4.9 4.7 4.4 4.2 4.0 3.6 3.4 3.2 3.0 2.6 latest krb),
      'topology' => %w(standalone replica-set sharded-cluster load-balanced),
      'auth-and-ssl' => %w(noauth-and-nossl auth-and-ssl),
    }
    @ruby_versions = %w(3.1 3.0 2.7 2.6 2.5 2.4 2.3 head jruby-9.3 jruby-9.2)
  ...
  get '/repos/:org/:repo/pulls/:id/submit-patch' do |org_name, repo_name, pull_id|
    ...
    diff = rc.diff_to_upstream_master(@pull.head_sha)
    # broken patch retrieved from ruby-git?
    diff += "\n"
    rc.apply_patch(text: diff)
    repo = system_fe.hit_repo(org_name, repo_name)
  ...
  get '/repos/:org/:repo/pulls/:id/close' do |org_name, repo_name, pull_id|
    @pull = gh_repo(org_name, repo_name).pull(pull_id)
  ...
  get '/repos/:org/:repo/pulls/:id/smart-add-label/oleg-check' do |org_name, repo_name, pull_id|
    ...
    remove = @pull.label_names.select { |name| %w(oleg-deferred oleg-done oleg-backlog).include?(name) }
    if remove.any?
  ...
  get '/repos/:org/:repo/pulls/:id/smart-add-label/oleg-done' do |org_name, repo_name, pull_id|
    ...
    remove = @pull.label_names.select { |name| %w(oleg-deferred oleg-check oleg-backlog).include?(name) }
    if remove.any?
      @pull.remove_label(*remove)
    end
